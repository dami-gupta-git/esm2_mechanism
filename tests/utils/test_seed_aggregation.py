...
Invariants:
- load_seed_files: loads every matching seed file as (basename, dict)
...
class TestLoadSeedFiles:
    ...
    def test_returns_basename_and_parsed_dict(self, tmp_path):
        _write_seed_file(tmp_path / "res_seed0.json", _seed_result(0.5))
        loaded = load_seed_files(str(tmp_path), "res_seed*.json")
        name, data = loaded[0]
    ...
    def test_corrupt_file_skipped(self, tmp_path):
        ...
        assert len(loaded) == 1
        assert loaded[0][0] == "res_seed0.json"
