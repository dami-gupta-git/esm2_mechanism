...
class TestWindowSequence:
    ...
    def test_short_sequence_unchanged(self):
        seq = "A" * 100
        result_seq, result_pos = window_sequence(seq, 50, window_half=500, max_len=1022)
        assert result_seq == seq
        assert result_pos == 50

    def test_exact_max_len_unchanged(self):
        seq = "A" * 1022
        result_seq, result_pos = window_sequence(seq, 511, window_half=500, max_len=1022)
        assert result_seq == seq
        assert result_pos == 511

    def test_long_sequence_windowed(self):
        seq = "A" * 2000
        result_seq, result_pos = window_sequence(seq, 1000, window_half=500, max_len=1022)
    ...
    def test_position_near_start(self):
        ...
        result_seq, result_pos = window_sequence(seq, 5, window_half=500, max_len=1022)
        assert result_seq[result_pos - 1] == "M"
        assert len(result_seq) <= 1022
    ...
    def test_position_near_end(self):
        ...
        result_seq, result_pos = window_sequence(seq, 1995, window_half=500, max_len=1022)
        assert result_seq[result_pos - 1] == "M"
        assert len(result_seq) <= 1022
    ...
    def test_windowed_length_does_not_exceed_max_len(self):
        ...
        for pos in [1, 100, 2500, 4900, 5000]:
            result_seq, _ = window_sequence(seq, pos, window_half=500, max_len=1022)
