...
class TestLoadRun:
    def test_flattens_nested_structures_to_dotted_paths(self, run_dir):
        _write(run_dir, "a.json", {"gates": {"K1": {"value": 0.891}}, "seeds": [1, 2]})
        leaves = load_run(run_dir)
        assert leaves["a.json.gates.K1.value"] == 0.891
...
class TestCompare:
    ...
    def test_boolean_change_is_categorical_not_numeric(self):
        old = {"f.json.gates.K2.passed": True}
        new = {"f.json.gates.K2.passed": False}
        result = compare(old, new, abs_threshold=0.005)
        assert len(result["changed"]) == 1
        assert result["moved"] == []
