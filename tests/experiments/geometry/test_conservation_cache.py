...
def _install_cache(monkeypatch, npy_path, meta_path):
    """Point the module at a cache written under tmp_path."""
    monkeypatch.setattr(conservation_axis, "CONS_CACHE", npy_path)
    monkeypatch.setattr(conservation_axis, "CONS_META", meta_path)


def _mock_model_load(monkeypatch, side_effect=None):
    """Stand in for torch and esm so no model is downloaded.

    Giving `side_effect` makes reaching the model load raise, which is how a test
    asserts that extraction got past the cache check rather than stopping at it.
    """
    ...
    if side_effect is not None:
        mock_esm.pretrained.load_model_and_alphabet.side_effect = side_effect
    monkeypatch.setitem(sys.modules, "esm", mock_esm)


def _cache_matching_its_inputs(tmp_path, monkeypatch, n, **metadata_overrides):
    """Variants, sequences, and an installed cache whose provenance matches them."""
    ...
    _install_cache(monkeypatch, npy_path, meta_path)
    return variants, seqs, metadata, npy_path
...
class TestLegacyCacheRejection:
    def test_no_fingerprint_raises(self, tmp_path, monkeypatch):
        ...
        n = 50
        variants = _make_variants(n)
        _install_cache(monkeypatch, *_write_cache(tmp_path, n))
        _mock_model_load(monkeypatch)
    ...
    def test_wrong_fingerprint_discards_and_continues(self, tmp_path, monkeypatch):
        ...
        n = 50
        variants = _make_variants(n)
        _install_cache(monkeypatch, *_write_cache(
            tmp_path, n, metadata={"n": n, "fingerprint": "wrong_fp"}
        ))
        _mock_model_load(
            monkeypatch,
            Exception("model load reached — cache was correctly discarded"),
        )
    ...
    def test_matching_fingerprint_accepted(self, tmp_path, monkeypatch):
        ...
        variants, seqs, _metadata, _npy = _cache_matching_its_inputs(
            tmp_path, monkeypatch, 50
        )
        _mock_model_load(
            monkeypatch,
            Exception(
...
class TestFullCacheProvenance:
    def test_changed_windowing_rejects_cache(self, tmp_path, monkeypatch):
        variants, seqs, metadata, npy_path = _cache_matching_its_inputs(
            tmp_path, monkeypatch, 4
        )
    ...
    def test_changed_array_content_rejects_cache(self, tmp_path, monkeypatch):
        variants, seqs, _metadata, npy_path = _cache_matching_its_inputs(
            tmp_path, monkeypatch, 4
        )
    ...
    def test_coverage_mismatch_rejects_cache(self, tmp_path, monkeypatch):
        variants, seqs, _metadata, _npy = _cache_matching_its_inputs(
            tmp_path, monkeypatch, 4, coverage=3
        )
