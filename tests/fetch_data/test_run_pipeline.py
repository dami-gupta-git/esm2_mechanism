...
def _run_main(argv: list[str], step_side_effects: dict[int, Exception | None] = None):
    ...
    with patch("sys.argv", ["run_pipeline"] + argv), \
         patch("esm2_mechanism.fetch_data.run_fetch_pipeline._make_steps", side_effect=fake_make_steps), \
         patch("esm2_mechanism.fetch_data.run_fetch_pipeline._save_state", side_effect=fake_save_state):
...
def test_auto_resume_no_prior_state(tmp_path):
    state_path = tmp_path / ".pipeline_state.json"
    with patch("esm2_mechanism.fetch_data.run_fetch_pipeline.STATE_FILE", state_path), \
         patch("esm2_mechanism.fetch_data.run_fetch_pipeline._load_state", return_value={}):
...
def test_auto_resume_from_last_completed(tmp_path):
    ...
    with patch("esm2_mechanism.fetch_data.run_fetch_pipeline.STATE_FILE", state_path), \
         patch("esm2_mechanism.fetch_data.run_fetch_pipeline._load_state", return_value=prior_state):
        mocks, saved, exit_code = _run_main([])
    assert exit_code == 0
    for n in range(1, 6):
...
def test_auto_resume_all_done(tmp_path, capsys):
    prior_state = {"last_completed_step": LAST_STEP, "last_completed_at": "2024-01-01T00:00:00"}
    with patch("esm2_mechanism.fetch_data.run_fetch_pipeline._load_state", return_value=prior_state):
        mocks, saved, exit_code = _run_main([])
    assert exit_code == 0
    for n in range(FIRST_STEP, LAST_STEP + 1):
        mocks[n].assert_not_called()
...
def test_from_step_invalid_value():
    with patch("esm2_mechanism.fetch_data.run_fetch_pipeline._load_state", return_value={}):
