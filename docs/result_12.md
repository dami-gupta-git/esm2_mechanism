...
This is the data collection phase (Phases 1 and 2 from `docs/plan_experiment.md`) preceding the modelling gate (Phase 3, Stage 1 / Gate 1). No model was trained here.

**Goal:** build a gene-level feature vector covering the proteome-context information hypothesised to carry the DN and HI signal that ESM-2 delta embeddings lack (result_10: ~half the family-split mechanism signal is fold memorisation, DN especially underperforms).
...
n_tissues_expressed (0% coverage) is excluded from the final matrix. PaxDb abundance was manually downloaded and achieves 98.4% coverage. The final matrix is 2,424 × 37 (8 continuous features × 4 derived columns each + 1 singleton indicator).
...
### Missing-data policy
For every continuous feature, a paired binary `<feature>_missing` indicator is appended. Missing values in the `.npy` matrix are median-imputed column-wise (fit on all 2,424 genes, not per fold — fold-level Z-scoring happens at modelling time in Phase 3). Raw NaN values are preserved in the TSV.

### Family-mean-centred residuals
For every continuous feature, a `<feature>_familyresid` column is computed: each gene's value minus the mean of genes in its Pfam family. Singleton families (genes with no Pfam family partner in the dataset) receive residual=0 and a `is_singleton_family=1` indicator. This was identified in the planning phase as the primary mitigation for the high-risk family-level leakage concern: proteome features like PPI_degree and constraint scores are correlated within protein families, and a model trained on absolute values could learn family identity rather than per-gene variation. Family-centred residuals force the model to use within-family deviations — the signal that should survive family-split CV — rather than cross-family absolute differences.
...
### gnomAD v4.1
No issues. The pilot's cached 95 MB TSV was reused. Column schema confirmed: gene=col 0, pLI=col 18 (`lof.pLI`), LOEUF=col 22 (`lof.oe_ci.upper`), mis_z=col 35 (`mis.z_score`), MANE-select=col 4 (`mane_select`). Selection logic: MANE-select transcript preferred; if absent, transcript with highest `lof.exp`.

### Paralogs
All 2,424 genes were in cache from the pilot (1,260 genes) or the extended REST fetch done in this phase (1,164 additional genes). Zero new REST calls needed on re-run. 100% coverage.

### Human Protein Atlas
The expected download URLs (`rna_tissue_category.tsv.gz`, `normal_tissue.tsv.gz`) returned 404. The current bulk download is `proteinatlas.tsv.zip` (6.7 MB compressed). Column `RNA tissue specificity` (text label: "Tissue enriched", "Group enriched", "Tissue enhanced", "Low tissue specificity", "Not detected") was mapped to a continuous proxy score: enriched→0.8, group→0.7, enhanced→0.6, low→0.2, not detected→0.0. 99.2% coverage. The `n_tissues_expressed` feature is not available from the bulk export (it would require the per-tissue normal_tissue TSV which is no longer served); treated as fully-missing.

### PaxDb
URL `https://pax-db.org/download/5.0/datasets/9606/9606-WHOLE_ORGANISM-integrated.txt` returned HTTP 403. The site is now JS-rendered; bulk downloads appear to require account authentication. Treated as fully-missing.

### BioPlex 3.0
Downloaded successfully (293T 10K Dec 2019 TSV, 12.9 MB). The column names are quoted in the file (`"SymbolA"`, `"SymbolB"`) — matched after stripping quotes via `csv.DictReader`. Degree = number of unique interaction partners in the network. 75% coverage (1,819/2,424 genes).

### ClinGen dosage
Downloaded from FTP (0.3 MB). File has no header row — all lines after comment block (`#`) are data. Columns by position: 0=gene symbol, 5=HI description text, 12=TS description text. Text labels mapped to integer scores: "Sufficient evidence for dosage pathogenicity"→3, "Some evidence"→2, "Emerging/little evidence"→1, "No evidence available"→0, AR-only genes/unevaluated→None. Low coverage expected and correct (19% HI, 37% TS) — ClinGen only curates a subset of clinically relevant genes; missingness carries information (uncurated genes are effectively "no strong evidence").

---

## What this does not include

- **Mathieson 2018 protein half-life** — supplementary table from a 2018 Nat Comms paper; not included in this automated pull. ~60% coverage expected. Would add turnover/stability information. Can be added manually if Phase 3 Gate 1 shows HPA tau is insufficient for the GOF signal.
- **PhosphoSitePlus PTM density** — requires registration. Not pursued; ClinGen HI/TS provides dosage sensitivity information that partially substitutes.
- **PaxDb abundance** — recovered via manual download. 98.4% coverage (2,385/2,424 genes). Automated download blocked (403 Forbidden); file placed manually at `data/9606-WHOLE_ORGANISM-integrated.txt`.
