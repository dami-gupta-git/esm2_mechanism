...
In plain terms: 63% of the apparent mechanism prediction signal that a standard evaluation would report is an artefact of how the data was split.

## Why it is exact across seeds

Different seeds shuffle the gene → fold and family → fold mappings, so both gene_split_F1 and family_split_F1 are noisy estimators that vary seed to seed. But their ratio (after subtracting chance) is invariant.
...
3. The protein family partition of the gene universe
...
5. **Cross-dataset comparison.** Does LF vary smoothly with within-family agreement rate? If yes, you get a rule of thumb: "datasets with within-family agreement > X% will have leakage fraction > Y%, and require family-aware CV."
...
The v2 section is the cheapest and lands the contribution within the existing publication arc. The standalone methods note has the highest field-impact potential per unit of effort once the analytic derivation is done.
