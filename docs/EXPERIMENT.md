...
**Moved to a dedicated file:** [`biorxiv/PREREGISTRATION_run_biorxiv.md`](../biorxiv/PREREGISTRATION_run_biorxiv.md).
Everything above this line is the original (run0-era) pre-registration and its outcomes and stays
in this file. The run_biorxiv rules (R7.1–R7.7: CI decision rule, confirmatory/exploratory split,
resampling unit, rare-class BCa, permutation budget, calibration note, and what would change the
conclusions), plus the run6 point estimates they are pinned against, now live in that file so they
sit apart from this document's narrative and cannot drift as `EXPERIMENT.md` is edited.
