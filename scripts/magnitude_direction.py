...
import multiseed_v1 as ms  # loaders, CV helpers, probes
...
def run_pathogenicity(pfam_map, seeds):
    ...
    for seed in seeds:
        gs = ms.gene_split_cv(genes, seed=seed)
...
def run_mechanism(pfam_map, seeds):
    ...
    print("=" * 60)
    dm, _dp, labels, genes = ms.load_geras(pfam_map)
    feats = decompose(dm)
...
def run_biophysical_direction(seeds):
    ...
    for fname in ("full", "dir"):
        per_seed = []
        for seed in seeds:
            splits = ms.gene_split_cv(proteins, seed=seed)  # group-holdout by protein
