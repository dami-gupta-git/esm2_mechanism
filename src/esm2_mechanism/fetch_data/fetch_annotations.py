...
Four pipeline steps:
  ...
  Step 1 — pfam  (run after fetch_variants --step merge)
    ...
    Input : data/merged_variants.json
  ...
  Step 3 — enzyme  (run after step 1 or independently)
    ...
    non-enzyme) from UniProt for all genes in merged_gene_list.tsv.
    Input : data/merged_variants.json, data/merged_gene_list.tsv
    Output: data/enzyme_labels.tsv

  Step 4 — alphamissense  (late stage — requires merged_valid_variants.json
    and pathogenicity_valid_variants.json produced by downstream scripts)
    Stream AlphaMissense bulk file; extract scores for target variants.
    Input : data/merged_valid_variants.json,
...
AM_MERGED_VALID_VARIANTS = DATA_DIR / "merged_valid_variants.json"
...
# ===========================================================================
# Step 4 — AlphaMissense scores
# ===========================================================================
def _build_am_gene_uniprot_map() -> dict[str, str]:
    with open(AM_MERGED_VALID_VARIANTS) as f:
        rows = json.load(f)

    # Count occurrences of each (gene, uniprot_id) pair across all variants,
    # then pick the most frequent UniProt ID per gene.
