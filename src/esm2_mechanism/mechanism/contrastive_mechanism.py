...
def train_projection_head(X_train, labels_train, gene_pfam_train,
                           le, hidden=(256, 64),
                           lr=1e-3, max_epochs=80, patience=12,
                           batch_size=512, margin=1.0, seed=42):
    import torch
    ...
    if len(anchors) < 50:
        print("  WARNING: too few cross-family triplets, falling back to all-family positives")
        ...
        anchors, positives, negatives = (np.array(anchors_fb),
                                          np.array(pos_fb), np.array(neg_fb))
    ...
    anc_tr = anchors[train_idx_mask]; pos_tr = positives[train_idx_mask]; neg_tr = negatives[train_idx_mask]
    anc_val = anchors[val_idx]; pos_val = positives[val_idx]; neg_val = negatives[val_idx]

    ds = TensorDataset(torch.tensor(anc_tr), torch.tensor(pos_tr), torch.tensor(neg_tr))
    loader = DataLoader(ds, batch_size=batch_size, shuffle=True)

    best_loss = float("inf"); patience_count = 0; best_state = None
    ...
    for epoch in range(max_epochs):
        ...
        if val_loss < best_loss - 1e-4:
            best_loss = val_loss; patience_count = 0
...
def run_cv(X, labels, genes, gene_pfam, pfam_map, le, splits, split_name,
    ...
    for fold_i, (train_idx, test_idx) in enumerate(splits):
        ...
        mu_raw = X_tr.mean(0); std_raw = X_tr.std(0) + 1e-8
        ...
        print(f"    raw k-NN:      macro_f1={raw_fm['macro_f1']:.3f}  "
        ...
        print(f"    contrastive:   macro_f1={cont_fm['macro_f1']:.3f}  "
...
def main():
    ...
    for split_name, split_key in [("Gene-split", "gene_split"), ("Family-split", "family_split")]:
        ...
        print(f"\n{split_name}:")
        print(f"  Contrastive k-NN:  macro_f1={cont.get('macro_f1_mean', float('nan')):.3f} ± {cont.get('macro_f1_std', float('nan')):.3f}"
        ...
        print(f"  Raw k-NN baseline: macro_f1={raw.get('macro_f1_mean', float('nan')):.3f} ± {raw.get('macro_f1_std', float('nan')):.3f}"
        ...
        delta_f1 = cont.get("macro_f1_mean", float("nan")) - raw.get("macro_f1_mean", float("nan"))
    ...
    if fam_cont_f1 > floor + 0.03:
        print(f"  ✓ Contrastive family-split F1 ({fam_cont_f1:.3f}) > MLP floor ({floor:.3f}) + 0.03")
        ...
        print(f"  ~ Contrastive ({fam_cont_f1:.3f}) > raw k-NN ({raw_fam_f1:.3f}) but below MLP floor ({floor:.3f})")
        ...
        print("    → Strong negative: even with explicit supervision to ignore family, ESM-2 deltas")
