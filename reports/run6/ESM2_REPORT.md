...
The short answer is that the delta encodes pathogenicity but not mechanism. Under rigorous family-split cross-validation, a linear probe on the delta classifies GOF/DN/LOF at the chance floor (macro-F1 ≈ 0.29). The apparent above-chance signal in standard gene-split evaluation comes from protein family recognition rather than from the mutation itself. A positive control confirms that the pipeline is sound: the same delta predicts ClinVar pathogenicity at AUROC 0.89 with no family-split drop. Within-family tests, where family identity cannot serve as a shortcut, show no recoverable mechanism signal. Scaling to ESM-3 1.4B raises the mechanism floor modestly (from 0.38 to 0.44), while structure tokens add little beyond sequence. Decomposing the pathogenicity delta reveals that it is essentially conservation: the model's own masked log-likelihood at the variant position matches or beats the full 1,280-dimensional embedding, and the embedding adds nothing on top.

---

### What is ESM-2?

ESM-2 is a protein language model trained on hundreds of millions of protein sequences. Like a language model trained on text, it learns to predict missing amino acids from context and, in doing so, builds up internal representations that capture something about protein biology: evolutionary conservation, structural propensity, and functional site identity.

The output of ESM-2 is an **embedding**, a vector of 1,280 numbers for each residue in the protein. These numbers encode the model's representation of that position in context. For a given variant, we compute two embeddings: one for the wildtype (normal) sequence and one for the mutant. The difference, known as the **delta**, captures how the mutation shifts the model's internal representation of the protein.
...
- **Gain-of-function (GOF)**: the mutant protein does too much. It may be constitutively active, resistant to normal inhibition, or acquire a new activity. GOF variants often act dominantly, so that a single mutant copy is enough to cause disease.
...
Knowing the mechanism matters clinically. A GOF variant might respond to an inhibitor, whereas a LOF variant might not. A DN variant can be dominant even when the gene is not haploinsufficient. Current variant interpretation tools (for example AlphaMissense and CADD) predict pathogenicity (whether a variant is damaging) but not mechanism (how it acts). These are different questions.

### The dataset

We use a merged dataset combining two sources. **Gerasimavicius et al. 2022** (*Nature Communications*) is a curated set of 10,233 missense variants across 948 disease genes, each labelled with a gene-level mechanism class (GOF, DN, or one of two LOF subtypes: haploinsufficient HI and autosomal recessive AR). Labels are derived from clinical genetics literature and ClinVar curation. This is the primary source of mechanism labels. **Gene2Phenotype (G2P)** is a database of gene-disease associations maintained by clinical genetics groups. We use the `molecular mechanism` field to assign mechanism labels to an additional set of genes not covered by Gerasimavicius.
...
Pathogenicity behaves as an angular property of ESM-2's perturbation space (what kind of disruption a mutation causes, not how large) and that angle is a single, family-universal direction. The decisive finding is what the direction turns out to be: conservation. ESM-2's mean-pooled embedding delta is, for pathogenicity, a worse and redundant re-encoding of the model's own masked-LM likelihood (0.859 versus 0.891). Pooling the embedding loses information the likelihood head exposes directly. This reframes the pathogenicity result as characterisation (the delta predicts pathogenicity because it partially reflects conservation) rather than a claim that the representation holds anything novel about damage beyond likelihood. Transferability is task-dependent within one frozen model: pathogenicity rides on a shared conservation axis that crosses family boundaries; mechanism has no comparably transferable direction.
...
The second priority is **ESM-3's function tokens**. The third ESM-3 modality was not tested because the function-conditioning path was not implemented in this run, not because the model lacks it (the open weights support it: `ESMProtein` accepts a `function_annotations` track). Function tokens condition on protein function annotations, and of the three modalities they are the most plausible pathway to mechanism signal: GOF/DN/LOF distinctions are function-level rather than structure-level, and the structure-token result in Section 6 already suggests that what is missing from sequence is not 3D geometry. One caveat tempers the expected payoff: function annotations describe the whole protein, so they are identical for the wildtype and the mutant and would largely cancel in the `mut − wt` delta (the same reason structure tokens added little). A fair test would likely need to depart from the delta-only framing (for example by probing the absolute or concatenated function-conditioned embeddings) rather than just adding the track.
