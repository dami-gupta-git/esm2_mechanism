...
Full run, all seven experiments.
...
**Embeddings are not re-extracted.** Nothing upstream of the probes changed, so every GPU embedding
step is skipped and the existing `.npy` arrays are reused in place. Embedding paths are keyed by
*model*, not run (`utils/paths.py:68-69`), so no copy is needed — and at ~10 GB, copying per run
would cost the space for no provenance gain. run_biorxiv result files record the embedding
fingerprint instead.
...
★ **Reference point, pinned.** There are two bars ESM-3 could be measured against, and they give
opposite answers. ESM-2's mechanism floor — chance level — is 0.380, and ESM-3 `seq` scores 0.438:

- against the bare floor 0.380, a lift of **+0.058** — comfortable, and an error bar on it would very
  likely stay clear of zero.
- against **0.430** (the floor plus a 0.05 "must beat chance by a useful amount" requirement), a
  margin of **+0.008** — thinner than a seed of spread, and an error bar on it will probably include
  zero.

The pre-registered gate recorded in `results/run6/esm3_mechanism/merged/summary.json` is
`family-split F1 > 0.430`, so that is the number the paper is on the hook for and **C5 asserts the
thin version: seq > floor + 0.05.** The two framings must never be mixed — the +0.058 lift cannot be
quoted as evidence while the 0.430 gate is the criterion. Every margin in the table below is measured
against 0.430.
...
M3 and K2 already **fail**, so a CI spanning zero reinforces those readings. The exposure is
concentrated in M2 and the contrastive gate, where a pass is claimed on a thin margin.
...
reading is *underpowered*, not *no effect*:
...
Deliverable: this section written into [`PREREGISTRATION_run_biorxiv.md`](PREREGISTRATION_run_biorxiv.md)
(a dedicated pre-registration file, split out from `docs/EXPERIMENT.md`'s "Pre-registered decisions"
section at line 166 so the run_biorxiv rules can't drift as `EXPERIMENT.md`'s narrative is edited)
...
uncorrected invites "you tested many things and reported the significant ones."

**Enumerate the confirmatory set up front — the handful the thesis actually rests on — and label
everything else exploratory.** Confirmatory set (five claims):
...
★ **Where "structure adds nothing" (M3) lives.** M3 asked whether adding structure to ESM-3 helps:
the gate required `seq_struct − seq > 0.030` and the observed gap is +0.014, so it fails, and
`ESM2_REPORT.md` §6 writes that up as "structure adds nothing."

That overstates it. Failing to clear a 0.030 bar is not the same as showing the effect is zero —
+0.014 is a positive gap, just not a large enough one to count. If its CI is wide enough to include
0.030, the data cannot tell a real 0.030-sized effect from nothing, which is what "underpowered to
detect an effect of the pre-registered size" means: too few genes to resolve a gap that small, not
...
falls below the measured floor plus 0.05 (the same margin as the M2 gate, so both read on one
scale), and is otherwise recorded as *not adjudicated*. The permutation test can refute C1 but
cannot confirm it.

C1 and C3 are the load-bearing pair (the dissociation) and C2 is the leakage account. If the
confirmatory set needs trimming, trim from C4/C5, not C1–C3.
...
`paired_cluster_bootstrap_diff` handles arms sharing one fold assignment: ESM-3 vs ESM-2, contrastive
...
10), and 0.418 (this panel). The ESM-3 M2 gate (seq 0.438 > base + 0.05) passes under 0.380 and
fails under the other two, so Section 6's conclusion depends on which copy is used. Tracked
separately.
...
across three reports; this collects every tested difference with its CI into one table.
